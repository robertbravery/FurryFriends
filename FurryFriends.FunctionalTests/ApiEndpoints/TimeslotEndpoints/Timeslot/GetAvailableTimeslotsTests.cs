...
public class GetAvailableTimeslotsTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    private readonly HttpClient _client;
    private const string BASE_URL = "/timeslots/available";

    public GetAvailableTimeslotsTests(CustomWebApplicationFactory<Program> factory)
    {
        _client = factory.CreateClient();
    }

    [Fact]
    public async Task GetAvailableTimeslots_ValidRequest_ReturnsSuccess()
    {
        // Arrange
        ...
        var result = await response.Content.ReadFromJsonAsync<Result<GetAvailableTimeslotsResponse>>();
    ...
    [Fact]
    public async Task GetAvailableTimeslots_NoAvailableSlots_ReturnsEmptyList()
    {
        // Arrange
        var petWalkerId = Guid.NewGuid();
        var date = DateOnly.FromDateTime(DateTime.Today.AddDays(1));
    ...
    [Fact]
    public async Task GetAvailableTimeslots_InvalidPetWalker_ReturnsNotFound()
    {
        // Arrange
        var petWalkerId = Guid.NewGuid();
        var date = DateOnly.FromDateTime(DateTime.Today.AddDays(1));
